...
cosmology = {"s":cosmo(70, 0, .3089, .6911), 
             ...
             "bs1":cosmo(70, 1, 0, 0), 
             "bs2":cosmo(70, -.5,.5 , 1),
             "conversion":cosmo(71,0, .3,.7) 
            }       #values gotten from http://adsabs.harvard.edu/abs/2015PASP..127...67B
...
raw = pd.read_csv('CSVResults.csv')
...
def radius(row):
    #converts c to Mpc/day and multiplies that by tau
    r = row['t_cent'] * (8.3942886772 *(10 ** -10))
    return float(r)

full['r_cm'] = full.apply(lambda row: radius(row), axis = 1)

def radius_e(row):
    r_e = row['t_c_perr'] * (8.3942886772 *(10 ** -10))
    return float(r_e)
