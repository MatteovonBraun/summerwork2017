...
full2 = pd.read_csv('SDSSRM_formatted.csv')

#This is so we can add the two dataframes cleanly and not have a devide by 0 error in ODR
full2['log_L_agn_err'] = 0.03 

full2 = full2[full2['Object'] != 778]
...
#this is so in the simulation later on we are only pulling from real values of L_err
error_l = pd.concat([full1,full3])
error_l = error_l.reset_index(drop=True)
...
sim_c = {'breakL0':44.62,
        ...
        'k1':1.241,
        'k2':-.249,
        'l_min':set_['log_L_agn'].min(),
        'l_max':set_['log_L_agn'].max(),
        'z_min':set_['z'].min(),
        'z_max':set_['z'].max(),
        't_min':set_['log_t_cent'].max(),
        't_max':set_['log_t_cent'].min()}
...
def dist(row,bounds):
    ...
    def pdf(l):
        ...
        L = (10**Decimal(l-breakL))
        D = 1/(L**Decimal(sim_c['beta']-1) + (L**Decimal(sim_c['alpha']-1)))
        return float(D)
    
    correction = quad(pdf, sim_c['l_min'], sim_c['l_max'])[0]
    ...
    dist_Log_L = my_pdf(momtype = 0, a = sim_c['l_min'] ,b=sim_c['l_max'],name='l_L_dist')
...
def simgen(bounds):
    ...
    def l_err(row, frame): #1 for luminosity, 2 for tau. Returns a value of error that adds together 
        
        
        frame = frame_mode(bounds)[0]
        column = 'log_L_agn_err'
        place = int(np.random.randint(len(frame.index), size=1))

        sig_ = frame[column][place]
        max_ = 5 * sig_
    ...
    def t_pert(row):
        ...
        tot_err = row['log_t_c_err']

#       pertibation generation 
        min_ = -t_raw
        max_ =  t_raw

        pertibation = norm_val(min_,max_,tot_err)
        t_ = t_raw + pertibation
        return t_
    ...
    def frame():
        ...
        func_app(dist,[['log_L_agn',[bounds]]],draft_)
        print get_time(), 'to simulate every luminosity.'
        func_app(L_44,[['log_L_agn_44']],draft_)
        func_app(l_err,[['log_L_agn_err',[frame_]]],draft_)

        draft['log_t_c_err'] = draft.apply(lambda row: t_err(row,frame_),axis = 1)
        draft['log_t_cent'] = draft.apply(lambda row: t_pert(row),axis = 1)
        draft['t_cent'] = draft.apply(lambda row: lin_t(row), axis = 1)
        draft['t_c_err'] = draft.apply(lambda row: lin_t_err(row), axis = 1)
...
def L_tau_plot():
    ...
    pl.errorbar(L2, t2,xerr=L_err2, yerr = t_err2, fmt = 'k.', ecolor='k'  , elinewidth= .5, label=Label2,zorder=10)
    pl.errorbar(L3, t3,xerr=L_err3, yerr = t_err3, fmt = 'r.', ecolor='r'  , elinewidth= .5, label=Label3,zorder=10)
    ...
    pl.axis([-3,3,0,3])
    fig.savefig('%d_obsz'%sim_n,bbox_inches='tight')
    plt.show()
