...
def flux(row):
    z = row['z']
    dis = float((lumdis(z, 'conversion'))/u.Mpc)*(3.08567758*10**18) #conversion to cm
...
def dis(row):
    ...
    x = m.sqrt(( (row['t_cent']*86400)/( 10**beta ) )**(1/alpha))   #converts tau from says to seconds so the units work out
    dis = c*x
    return dis
